...
: >prn 32 over over - 0< if 46 emit drop drop exit then drop 127 1 - over over swap - 0< if 46 emit drop drop exit then drop emit ;
: delay drop 1234 0 do 1 drop loop ;
: ecol 58 emit ;
: hadr dup 1 + h. ecol space ;
: rhlist hadr 16 + dup 16 - over over do 1 + over over swap - 1 - 0< if dup rbyte dup 16 - 0< if 48 emit then h. 100 delay then loop drop ;
: ralist space space 16 + dup 16 - over over do 1 + over over swap - 1 - 0< if dup rbyte >prn 100 delay then loop drop ;
...
: sbl dup blist drop ;
: tellme drop ;
: tstbb 144 kst ! 155 kbi ! 74 tellme ;
: k-- swap 1 - bmk and bmk 2 - over over swap - 0< if swap 1 - bmk and 1 - bmk and 1 - bmk and swap then drop dup 0= if 1 - bmk and bmk 2 - then swap ;
: k++ kst @ 199 - 0= if k-- k-- then swap 1 + bmk and bmk 2 - over over swap - 0< if swap 1 + bmk and 1 + bmk and 1 + bmk and swap then drop swap ;
...
: hco kst @ 31 - 0= if kbi @ 1 - kbi ! 99 tellme 99 tellme drop drop swap 1 - swap 1 1 10 tellme 100 tellme drop drop sbl then 109 tellme ;
: hbk kst @ 199 - 0= if 1 drop then ;
: sam lxa bfc swap k++ over over + begin kbi @ 1 + kbi ! 254 kst ! 1 drop key ctrl! bksp! escp! hes hbk kst @ 254 - 0= if swap c! then kst @ 199 - 0= if drop drop then 77 tellme hco k++ over over + 1 drop again ;
